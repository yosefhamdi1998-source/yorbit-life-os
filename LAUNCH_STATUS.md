...
- Separately found, not yet fixed: disconnecting a single bank account only updates local status - it never calls Plaid's item/remove, so that access token stays valid at Plaid indefinitely (account deletion is unaffected; it revokes independently). Pure engineering, not approval-gated - see OWNER_ACTIONS.md item 1.
