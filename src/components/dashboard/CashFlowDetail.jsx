...
export default function CashFlowDetail({ bucket, onClose }) {
      ...
      {bucket && <>
        <div className="grid grid-cols-3 gap-2 rounded-xl bg-secondary/60 p-3">
          {[['Income', bucket.income, 'text-emerald-600 dark:text-emerald-400'], ['Spending', bucket.expense, 'text-orange-600 dark:text-orange-400'], ['Net', bucket.net, 'text-foreground']].map(([label, value, color]) =>
            <div key={label} className="min-w-0"><p className="text-xs text-muted-foreground">{label}</p><p className={`font-bold tabular-nums break-words text-sm sm:text-xl ${color}`}>{money(value)}</p></div>)}
        </div>
        <div>
          ...
          <div className="space-y-3">{categories.slice(0, 6).map(([name, amount]) => <div key={name}>
            <div className="flex justify-between gap-3 text-sm mb-1"><span className="capitalize">{name.replaceAll('_', ' ')}</span><span className="tabular-nums font-medium">{money(amount)}</span></div>
