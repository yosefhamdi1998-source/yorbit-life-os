...
export default function Layout() {
  ...
  return (
    <div className="min-h-screen flex overflow-x-hidden w-full">
      ...
      {/* ── Desktop sidebar ─────────────────────────────────────────── */}
      {/* Just the 6 primary tabs as lined-up rows, same set as the phone's
          bottom tab bar — not the old 16-item list. Everything else is one
          click away through the More row, same as it already works on
          phones via the top bar's grid icon. */}
      <aside className="hidden lg:flex flex-col w-56 border-r border-border/50 fixed inset-y-0 left-0 z-40 bg-card">
        ...
        <nav className="flex-1 px-2 py-3 space-y-0.5 overflow-y-auto">
          ...
          >
            <Grid2x2 className="w-4 h-4 shrink-0" strokeWidth={1.8} />
            More
          </NavLink>
        </nav>
      ...
      >
        {/* Reset top padding on desktop since the sidebar has no top bar */}
