...
> **Latest verified status - September 23, 2026:** See LAUNCH_STATUS.md and OWNER_ACTIONS.md for current evidence and exact remaining actions. Web security, ownership, import/export and bank holdings repairs are deployed. Router security and missing native plugin packaging fixes passed the complete local suite, lint, build and source sync; deployment is recorded separately. Native builds now require Xcode/iOS SDK 26+, but no signed build, physical-device run, TestFlight or App Store submission was performed. Payment/entitlement/cancellation/deletion end-to-end verification and native bank return remain open. The historical sections below are not current certification.

> **Current verification note — September 11, 2026:** The September 5 audit below is historical, not a current certification. See YORBIT_PROGRESS.md for subsequent fixes and actual test coverage. Native purchase pricing, availability, retry, cancellation, and access feedback now have mocked/source-level regression coverage; no signed iPhone build or live Apple purchase has been verified here. RevenueCat's public Apple key and App Store ID now read public build environment settings. The iOS workflow checks APP_STORE_APPLE_ID and VITE_REVENUECAT_APPLE_API_KEY before building and maps the Apple ID to VITE_APP_STORE_ID. This validates presence/format only; actual account configuration, products and entitlements remain unverified. The Codemagic workflow now targets Node 22 and the checked-in Swift-package Xcode project. A signed macOS build is still unverified; the workflow now references uploaded App Store signing identities and its environment credentials consistently. The matching certificate/profile, numeric APP_STORE_APPLE_ID, and Apple credentials still need verified account configuration. Native workflows are manual so website pushes do not trigger native build costs or submissions. Build numbering now stops on a missing ID or unverifiable Apple build number. Bank-token protection and production billing verification remain launch blockers.
