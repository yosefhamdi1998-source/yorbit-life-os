...
// Backfill mode: emit UPDATEs that attach occurred_at to rows already
// imported, matched on the export's own id. Writes nothing else - no
// inserts, no deletes - so it is safe to run against a populated table.
const TIMES_OUT = args.includes('--times') ? args[args.indexOf('--times') + 1] : null;
...
function readExport(file) {
  ...
  for (let i = hi + 1; i < lines.length; i++) {
    ...
    rows.push({
      ...
      // Full instant, kept because FIFO cost basis needs the order of
      // trades WITHIN a day. Discarding it moved realized P&L by
      // $26,085.82 across 2018-2023 and flipped its sign. Coinbase
      // writes '2023-12-29 05:34:34 UTC'; Postgres parses that as-is.
      occurred_at: (f[C.ts] || '').trim() || null,
...
// --- write --------------------------------------------------------------
if (TIMES_OUT) {
  // Only rows that actually carry a timestamp. A row without one keeps
  // occurred_at NULL rather than being given a fabricated midnight - the
  // P&L functions fall back to date ordering for those and the UI reports
  // the coverage, so a partial backfill reads as partial.
  const timed = all.filter(r => r.occurred_at);
  const esc = (v) => "'" + String(v).replace(/'/g, "''") + "'";
  const out = [
    '-- generated by scripts/import-coinbase.js --times',
    '-- Attaches the trade time to rows already imported. UPDATE only.',
    '-- Matched on provider_transaction_id, which is the id Coinbase assigned.',
    'begin;',
  ];
  for (let i = 0; i < timed.length; i += 500) {
    const chunk = timed.slice(i, i + 500);
    out.push('update transactions t set occurred_at = v.ts::timestamptz');
    out.push('from (values');
    out.push(chunk.map(r => '(' + esc(r.provider_transaction_id) + ',' + esc(r.occurred_at) + ')').join(',\n'));
    out.push(') as v(pid, ts)');
    out.push('where t.provider_transaction_id = v.pid and t.occurred_at is distinct from v.ts::timestamptz;');
  }
  out.push('commit;');
  fs.writeFileSync(TIMES_OUT, out.join('\n'));
  console.log('\nWrote ' + TIMES_OUT + ' (' + timed.length + ' of ' + all.length + ' rows carry a timestamp)');
  process.exit(0);
}
...
if (SQL_OUT) {
  ...
  for (let i = 0; i < all.length; i += 400) {
    ...
    out.push('insert into transactions (user_id,title,amount,type,category,date,occurred_at,provider_transaction_id,import_source,provider_memo,crypto_asset,crypto_quantity,exclusion_reason,exclude_from_budget) values');
    out.push(chunk.map(r => {
      const meta = TYPES[r.tx_type] || { direction: 'expense' };
      const amt = Math.abs(r.total) || Math.abs(r.quantity * r.price) || 0;
      return '(' + UID + ',' + esc('Coinbase ' + r.tx_type + (r.asset ? ' ' + r.asset : '')) + ',' + num(amt) + ',' + esc(meta.direction) + ",'investment'," + esc(r.date) + ',' + (r.occurred_at ? esc(r.occurred_at) + '::timestamptz' : 'null') + ',' + esc(r.provider_transaction_id) + ',' + esc('csv:coinbase:' + r.account_id.slice(0,8)) + ',' + esc(r.notes || null) + ',' + esc(r.asset || null) + ',' + num(r.quantity) + ",'investment',true)";
    }).join(',\n'));
