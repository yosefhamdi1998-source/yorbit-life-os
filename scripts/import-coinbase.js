...
console.log('\n' + '-'.repeat(60));
if (negatives.length) {
  console.log(`RECONSTRUCTION FAILED: negative balance in ${negatives.join(', ')}.`);
  console.log('A negative holding is impossible, so the export is missing activity.');
  console.log('Showing nothing rather than a number that cannot be true.');
} else {
  console.log(`All crypto balances non-negative. Estimated value $${estTotal.toFixed(2)}`);
  console.log('Price is the most recent "Price at Transaction", NOT a live market');
  console.log('price - as stale as that trade. An estimate, not a portfolio value.');
}

// --- write --------------------------------------------------------------
if (!APPLY) {
  console.log('\nDRY RUN - nothing written. Re-run with --apply to import.');
  process.exit(negatives.length ? 1 : 0);
}
